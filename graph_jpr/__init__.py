    ...
    def _validate_params(self, X, lambdas, rho, n_lambdas, cv_folds, criterion):
        ...
        if use_model_selection:
            if cv_folds > 0 and criterion != '':
                raise ValueError('Cannot use cross-validation and AIC/BIC at the same time')
            elif len(lambdas) > 0 and n_lambdas > 0:
                raise ValueError('Cannot pre-specify lambdas and specify n_lambdas')
            elif len(lambdas) == 0 and n_lambdas == 0:
                raise ValueError('Must specify lambdas or n_lambdas when using CV or AIC/BIC')
            ...
            if len(lambdas) != X.shape[1]:
                raise ValueError('lambdas must be scalar or the same length as the number of features when not using CV or AIC/BIC')
            
            if n_lambdas > 0:
                raise ValueError('Cannot specify n_lambdas when not using CV or AIC/BIC')
...
class JointPartialRegression(Estimator):
    ...
    def fit(self, X: np.array, lambdas: Union[float, Iterable[float]]=[], rho: float=0.0, n_lambdas: int=0, cv_folds: int=0, criterion: str=''):
        """
        ...
        self.precision_, self.intercepts_, self.selected_lambdas_, self.lambdas_grid_ = fit_jpr(X, lambdas, rho, self.fit_intercept, n_lambdas, cv_folds, criterion, self.max_iter, self.tol)
...
class SparseHuber(Estimator):
    ...
    def fit(self, X: np.array, y: np.array, lambdas: Union[float, Iterable[float]], rho: float, n_lambdas: int=0, cv_folds: int=0, criterion: str='', theta_start=None, intercept_start=None):
        """
        Fit the model to the data.
        ...
        self.coef_, self.intercept_, self.selected_lambda_, self.lambda_grid = fit_sparse_huber(X, y, lambdas, rho, self.fit_intercept, n_lambdas, cv_folds, criterion, alpha, self.max_iter, self.tol, theta_start=theta_start)
